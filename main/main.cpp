...
// Wi-Fi, MQTT, GPIO, LEDC servo control, timers, usage tracking and NimBLE NUS are native ESP-IDF APIs.
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <algorithm>
...
// Hardware mapping retained from Firm.ino.
...
static constexpr gpio_num_t SWITCH_PIN=GPIO_NUM_33, LED_WIFI=GPIO_NUM_25, LED_MQTT=GPIO_NUM_26, LED_BT=GPIO_NUM_27;
static constexpr gpio_num_t FAN1_SERVO_PIN=GPIO_NUM_14, FAN2_SERVO_PIN=GPIO_NUM_32;
...
static bool wifiConnecting=false,wifiScanActive=false,syncRequested=false;
...
// MQTT protocol retained from Firm.ino.
...
// BLE NUS UUIDs retained from Firm.ino.
static constexpr uint16_t MAX_BLE_CLIENTS=5;
static uint16_t bleClients[MAX_BLE_CLIENTS]={BLE_HS_CONN_HANDLE_NONE};
...
static void bleQueuePush(const char *msg){
    if(!msg)return;
    xSemaphoreTake(bleMutex,portMAX_DELAY);
    int next=(bleHead+1)%BLE_QUEUE_SIZE;
    if(next==bleTail)bleTail=(bleTail+1)%BLE_QUEUE_SIZE;
    snprintf(bleQueue[bleHead],BLE_MSG_SIZE,"%s",msg); bleHead=next;
    xSemaphoreGive(bleMutex);
}
static void bleNotifyAll(const char *msg){
    if(!bleRunning||!clientCount||!bleTxHandle)return;
    size_t len=strnlen(msg,240);
    for(int i=0;i<MAX_BLE_CLIENTS;i++){
        if(bleClients[i]==BLE_HS_CONN_HANDLE_NONE||!bleClientSubscribed[i])continue;
        struct os_mbuf *om=ble_hs_mbuf_from_flat(msg,len);
        if(!om)continue;
        if(ble_gatts_notify_custom(bleClients[i],bleTxHandle,om)!=0)os_mbuf_free_chain(om);
    }
}
static void processBLEQueue(){
    static uint64_t last=0; uint64_t now=nowMs();
    if(now-last<20||!bleRunning)return;
    char msg[BLE_MSG_SIZE]{}; bool have=false;
    xSemaphoreTake(bleMutex,portMAX_DELAY);
    if(bleTail!=bleHead){memcpy(msg,bleQueue[bleTail],BLE_MSG_SIZE);bleTail=(bleTail+1)%BLE_QUEUE_SIZE;have=true;}
    xSemaphoreGive(bleMutex);
    if(have){bleNotifyAll(msg);last=now;}
}
...
static void sendFullStateSync(){sendRelayMsg();sendFanMsg();}

// Native LEDC replacement for ESP32Servo. Same 50 Hz and 500..2400 us pulse range.
...
static void updateLEDs(){uint64_t n=nowMs();if(n-lastBlink>500){blinkState=!blinkState;lastBlink=n;}bool w=false,m=false,b=false;if(state==SystemState::WIFI_START||wifiScanActive||wifiConnecting)w=blinkState;else if(state==SystemState::WIFI_MODE)w=true;if(state==SystemState::WIFI_MODE)m=mqttConnected?true:blinkState;if(state==SystemState::BT_MODE)b=clientCount>0;else b=false;gpio_set_level(LED_WIFI,w);gpio_set_level(LED_MQTT,m);gpio_set_level(LED_BT,b);}
...
{.type=BLE_GATT_SVC_TYPE_PRIMARY,.uuid=&serviceUuid.u,.characteristics=(struct ble_gatt_chr_def[]){
{.uuid=&txUuid.u,.access_cb=bleGattAccess,.val_handle=&bleTxHandle,.flags=BLE_GATT_CHR_F_NOTIFY},
{.uuid=&rxUuid.u,.access_cb=bleGattAccess,.flags=BLE_GATT_CHR_F_WRITE|BLE_GATT_CHR_F_WRITE_NO_RSP},{0}}},{0}};
...
static void startBLE(){if(bleRunning||!bleReady)return;bleRunning=true;struct ble_gap_adv_params a{};a.conn_mode=BLE_GAP_CONN_MODE_UND;a.disc_mode=BLE_GAP_DISC_MODE_GEN;ble_gap_adv_start(ownAddrType,nullptr,BLE_HS_FOREVER,&a,bleGapEvent,nullptr);ESP_LOGI(TAG,"BLE advertising started");}
static void stopBLE(){if(!bleRunning)return;bleRunning=false;ble_gap_adv_stop();ESP_LOGI(TAG,"BLE advertising stopped");}
