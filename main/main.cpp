...
static void mqttEvent(void*,esp_event_base_t,int32_t eventId,void *eventData){auto *event=(esp_mqtt_event_handle_t)eventData;switch((esp_mqtt_event_id_t)eventId){case MQTT_EVENT_CONNECTED:mqttConnected=true;esp_mqtt_client_subscribe(event->client,TOPIC_CMD,1);publish(TOPIC_WELCOME,"ESP32 online",1,true);sendRelayMsg();sendWiFiMsg();sendFanMsg();ESP_LOGI(TAG,"MQTT connected");break;case MQTT_EVENT_DISCONNECTED:mqttConnected=false;break;case MQTT_EVENT_DATA:{int len=event->data_len;if(len>95)len=95;char msg[96];memcpy(msg,event->data,len);msg[len]=0;handleCommand(msg);break;}default:break;}}
...
static void stopMQTT(){mqttConnected=false;if(mqtt&&mqttStarted){(void)esp_mqtt_client_stop(mqtt);mqttStarted=false;}}
...
static int performWifiScan(){wifi_scan_config_t scan={};scan.show_hidden=false;if(esp_wifi_scan_start(&scan,true)!=ESP_OK)return -1;uint16_t count=0;if(esp_wifi_scan_get_ap_num(&count)!=ESP_OK||count==0)return -1;std::vector<wifi_ap_record_t> records(count);if(esp_wifi_scan_get_ap_records(&count,records.data())!=ESP_OK)return -1;return findBestSavedNetworkFromScan(count,records.data());}
...
static int bleAccess(uint16_t conn,uint16_t attr,struct ble_gatt_access_ctxt *ctxt,void*){if(ctxt->op!=BLE_GATT_ACCESS_OP_WRITE_CHR)return BLE_ATT_ERR_UNLIKELY;char msg[BLE_MSG_SIZE]={};uint16_t len=ctxt->om->om_len;if(len>=sizeof(msg))len=sizeof(msg)-1;if(ble_hs_mbuf_to_flat(ctxt->om,msg,len,nullptr)!=0)return BLE_ATT_ERR_UNLIKELY;msg[len]=0;handleCommand(msg);(void)conn;(void)attr;return 0;}
static int bleGap(struct ble_gap_event *event,void*){if(event->type==BLE_GAP_EVENT_CONNECT){if(event->connect.status==0){if(clientCount>=MAX_BLE_CLIENTS)ble_gap_terminate(event->connect.conn_handle,BLE_ERR_CONN_LIMIT);else{connectedClients[clientCount]=event->connect.conn_handle;notifyEnabled[clientCount]=false;clientCount++;syncRequested=true;}}}else if(event->type==BLE_GAP_EVENT_DISCONNECT){int slot=bleClientSlot(event->disconnect.conn.conn_handle);if(slot>=0){for(int i=slot;i<clientCount-1;i++){connectedClients[i]=connectedClients[i+1];notifyEnabled[i]=notifyEnabled[i+1];}connectedClients[clientCount-1]=INVALID_CONN_HANDLE;notifyEnabled[clientCount-1]=false;if(clientCount>0)clientCount--;}if(bleRunning){struct ble_gap_adv_params p={};p.conn_mode=BLE_GAP_CONN_MODE_UND;p.disc_mode=BLE_GAP_DISC_MODE_GEN;ble_gap_adv_start(bleAddrType,nullptr,BLE_HS_FOREVER,&p,bleGap,nullptr);}}else if(event->type==BLE_GAP_EVENT_SUBSCRIBE){int slot=bleClientSlot(event->subscribe.conn_handle);if(slot>=0&&event->subscribe.attr_handle==txHandle)notifyEnabled[slot]=event->subscribe.cur_notify;}return 0;}
...
static void stopBLE(){if(!bleRunning)return;ble_gap_adv_stop();bleRunning=false;for(int i=0;i<MAX_BLE_CLIENTS;i++){connectedClients[i]=INVALID_CONN_HANDLE;notifyEnabled[i]=false;}clientCount=0;}
