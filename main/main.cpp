...
static char mqttRxBuffer[96];static size_t mqttRxLength=0;static bool mqttRxOverflow=false;
...
static int bleAccess(uint16_t conn,uint16_t attr,struct ble_gatt_access_ctxt *ctxt,void*){if(ctxt->op!=BLE_GATT_ACCESS_OP_WRITE_CHR)return BLE_ATT_ERR_UNLIKELY;char msg[BLE_MSG_SIZE]={};uint16_t len=OS_MBUF_PKTLEN(ctxt->om);if(len>=sizeof(msg))return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;if(ble_hs_mbuf_to_flat(ctxt->om,msg,len,nullptr)!=0)return BLE_ATT_ERR_UNLIKELY;msg[len]=0;handleCommand(msg);(void)conn;(void)attr;return 0;}
static void startBLEAdvertising(){if(!bleRunning||!bleReady||bleStopping||clientCount>=MAX_BLE_CLIENTS)return;if(ble_gap_adv_active())return;struct ble_gap_adv_params p={};p.conn_mode=BLE_GAP_CONN_MODE_UND;p.disc_mode=BLE_GAP_DISC_MODE_GEN;p.itvl_min=20;p.itvl_max=40;int rc=ble_gap_adv_start(bleAddrType,nullptr,BLE_HS_FOREVER,&p,bleGap,nullptr);if(rc!=0&&rc!=BLE_HS_EALREADY)ESP_LOGW(TAG,"BLE advertising start failed: %d",rc);}
static int bleGap(struct ble_gap_event *event,void*){
if(event->type==BLE_GAP_EVENT_CONNECT){if(event->connect.status==0){if(clientCount>=MAX_BLE_CLIENTS){(void)ble_gap_terminate(event->connect.conn_handle,BLE_ERR_CONN_LIMIT);}else{int slot=clientCount;connectedClients[slot]=event->connect.conn_handle;notifyEnabled[slot]=false;bleSyncPending[slot]=true;clientCount++;if(bleRunning&&!bleStopping)startBLEAdvertising();}}else if(bleRunning&&!bleStopping)startBLEAdvertising();}
else if(event->type==BLE_GAP_EVENT_DISCONNECT){int slot=bleClientSlot(event->disconnect.conn.conn_handle);if(slot>=0){for(int i=slot;i<clientCount-1;i++){connectedClients[i]=connectedClients[i+1];notifyEnabled[i]=notifyEnabled[i+1];bleSyncPending[i]=bleSyncPending[i+1];}if(clientCount>0)clientCount--;if(clientCount<MAX_BLE_CLIENTS){connectedClients[clientCount]=INVALID_CONN_HANDLE;notifyEnabled[clientCount]=false;bleSyncPending[clientCount]=false;}}if(bleStopping&&clientCount==0)bleStopping=false;if(bleRunning&&!bleStopping&&clientCount<MAX_BLE_CLIENTS)startBLEAdvertising();}
else if(event->type==BLE_GAP_EVENT_ADV_COMPLETE){if(bleRunning&&!bleStopping)startBLEAdvertising();}
else if(event->type==BLE_GAP_EVENT_SUBSCRIBE){int slot=bleClientSlot(event->subscribe.conn_handle);if(slot>=0&&event->subscribe.attr_handle==txHandle){notifyEnabled[slot]=event->subscribe.cur_notify;if(notifyEnabled[slot])bleSyncPending[slot]=true;}}
return 0;}
static int bleInitServices(){static struct ble_gatt_chr_def chars[]={{.uuid=&CHARACTERISTIC_TX.u,.access_cb=nullptr,.arg=nullptr,.descriptors=nullptr,.flags=BLE_GATT_CHR_F_NOTIFY,.min_key_size=0,.val_handle=&txHandle,.cpfd=nullptr},{.uuid=&CHARACTERISTIC_RX.u,.access_cb=bleAccess,.arg=nullptr,.descriptors=nullptr,.flags=BLE_GATT_CHR_F_WRITE|BLE_GATT_CHR_F_WRITE_NO_RSP,.min_key_size=0,.val_handle=nullptr,.cpfd=nullptr},{0}};static struct ble_gatt_svc_def services[]={{.type=BLE_GATT_SVC_TYPE_PRIMARY,.uuid=&SERVICE_UUID.u,.includes=nullptr,.characteristics=chars},{0}};int rc=ble_gatts_count_cfg(services);if(rc)return rc;return ble_gatts_add_svcs(services);}
static void bleSync(){if(ble_hs_id_infer_auto(0,&bleAddrType)!=0)return;ble_svc_gap_device_name_set("RanjanaSmartHome");ble_att_set_preferred_mtu(247);struct ble_hs_adv_fields fields={};fields.flags=BLE_HS_ADV_F_DISC_GEN|BLE_HS_ADV_F_BREDR_UNSUP;const char *name="RanjanaSmartHome";fields.name=(uint8_t*)name;fields.name_len=strlen(name);fields.name_is_complete=1;fields.uuids128=(ble_uuid128_t*)&SERVICE_UUID;fields.num_uuids128=1;fields.uuids128_is_complete=1;if(ble_gap_adv_set_fields(&fields)==0)bleReady=true;startBLEAdvertising();}
...
static void bleReset(int reason){ESP_LOGW(TAG,"NimBLE host reset: reason=%d",reason);bleRunning=false;bleReady=false;bleStopping=false;clientCount=0;syncRequested=false;bleResetPending=true;for(int i=0;i<MAX_BLE_CLIENTS;i++){connectedClients[i]=INVALID_CONN_HANDLE;notifyEnabled[i]=false;bleSyncPending[i]=false;}}
...
static void stopBLE(){if(!bleRunning&&!bleStopping)return;(void)ble_gap_adv_stop();bleRunning=false;bleStopping=true;for(int i=0;i<MAX_BLE_CLIENTS;i++){uint16_t handle=connectedClients[i];if(handle!=INVALID_CONN_HANDLE)(void)ble_gap_terminate(handle,BLE_ERR_REM_USER_CONN_TERM);}if(clientCount==0)bleStopping=false;}
static void processBLEQueue(){if(!bleQueue||!bleRunning||clientCount==0)return;static uint64_t lastSend=0;if(nowMs()-lastSend<20)return;BleMessage message;if(xQueueReceive(bleQueue,&message,0)!=pdTRUE)return;const size_t messageLen=strnlen(message.data,sizeof(message.data));for(int i=0;i<MAX_BLE_CLIENTS;i++){uint16_t conn=connectedClients[i];if(conn==INVALID_CONN_HANDLE||!notifyEnabled[i])continue;struct os_mbuf *om=ble_hs_mbuf_from_flat(message.data,messageLen);if(!om){ESP_LOGW(TAG,"BLE TX buffer allocation failed");continue;}int rc=ble_gatts_notify_custom(conn,txHandle,om);if(rc!=0){ESP_LOGW(TAG,"BLE notify failed conn=%u rc=%d",conn,rc);os_mbuf_free_chain(om);}}lastSend=nowMs();}
