...
if(event->type==BLE_GAP_EVENT_CONNECT){
  ...
  if(slot>=0){
    for(int i=slot;i<clientCount-1;i++){connectedClients[i]=connectedClients[i+1];notifyEnabled[i]=notifyEnabled[i+1];bleSyncPending[i]=bleSyncPending[i+1];bleSyncPending[i]=bleSyncPending[i+1];}
