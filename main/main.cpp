...
static constexpr int NUM_WIFI=4,NUM_RELAYS=8,MAX_BLE_CLIENTS=5,BLE_QUEUE_SIZE=30,BLE_MSG_SIZE=220;
...
static bool lastSwitchState=true,blinkState=false,wifiScanActive=false,syncRequested=false,wifiConnecting=false,wifiConnected=false,mqttConnected=false,mqttStarted=false,bleRunning=false,bleReady=false;
...
static void bleQueuePush(const char *msg){if(!bleQueue||!bleRunning||clientCount==0||!msg)return;BleMessage m={};snprintf(m.data,sizeof(m.data),"%s",msg);(void)xQueueSend(bleQueue,&m,0);}
...
static void mqttEvent(void*,esp_event_base_t,int32_t eventId,void *eventData){auto *event=(esp_mqtt_event_handle_t)eventData;switch((esp_mqtt_event_id_t)eventId){case MQTT_EVENT_CONNECTED:mqttConnected=true;mqttResetRx();esp_mqtt_client_subscribe(event->client,TOPIC_CMD,1);publish(TOPIC_WELCOME,"ESP32 online",1,true);sendRelayMsg();sendWiFiMsg();sendFanMsg();ESP_LOGI(TAG,"MQTT connected");break;case MQTT_EVENT_DISCONNECTED:mqttConnected=false;mqttResetRx();break;case MQTT_EVENT_DATA:{if(event->current_data_offset==0)mqttResetRx();if(event->current_data_offset!=mqttRxLength)mqttRxLength=event->current_data_offset;if(mqttRxLength+event->data_len>=sizeof(mqttRxBuffer)){mqttRxOverflow=true;}else if(!mqttRxOverflow){memcpy(mqttRxBuffer+mqttRxLength,event->data,event->data_len);mqttRxLength+=event->data_len;mqttRxBuffer[mqttRxLength]=0;}if(event->current_data_offset+event->data_len>=event->total_data_len){if(!mqttRxOverflow)mqttRxBuffer[mqttRxLength]=0;else mqttRxBuffer[0]=0;if(!mqttRxOverflow)mqttRxLength=strlen(mqttRxBuffer);if(!mqttRxOverflow&&mqttRxLength>0)handleCommand(mqttRxBuffer);mqttResetRx();}break;}default:break;}}
...
static int performWifiScan(){wifi_scan_config_t scan={};scan.show_hidden=false;if(esp_wifi_scan_start(&scan,true)!=ESP_OK)return -1;uint16_t count=0;if(esp_wifi_scan_get_ap_num(&count)!=ESP_OK||count==0){(void)esp_wifi_clear_ap_list();return -1;}std::vector<wifi_ap_record_t> records(count);if(esp_wifi_scan_get_ap_records(&count,records.data())!=ESP_OK){(void)esp_wifi_clear_ap_list();return -1;}return findBestSavedNetworkFromScan(count,records.data());}
...
static void startBLEAdvertising(){if(!bleRunning||!bleReady)return;struct ble_gap_adv_params p={};p.conn_mode=BLE_GAP_CONN_MODE_UND;p.disc_mode=BLE_GAP_DISC_MODE_GEN;int rc=ble_gap_adv_start(bleAddrType,nullptr,BLE_HS_FOREVER,&p,bleGap,nullptr);if(rc!=0&&rc!=BLE_HS_EALREADY)ESP_LOGW(TAG,"BLE advertising start failed: %d",rc);}
static int bleGap(struct ble_gap_event *event,void*){if(event->type==BLE_GAP_EVENT_CONNECT){if(event->connect.status==0){if(clientCount>=MAX_BLE_CLIENTS)ble_gap_terminate(event->connect.conn_handle,BLE_ERR_CONN_LIMIT);else{connectedClients[clientCount]=event->connect.conn_handle;notifyEnabled[clientCount]=false;clientCount++;syncRequested=true;startBLEAdvertising();}}else if(bleRunning){startBLEAdvertising();}}else if(event->type==BLE_GAP_EVENT_DISCONNECT){int slot=bleClientSlot(event->disconnect.conn.conn_handle);if(slot>=0){for(int i=slot;i<clientCount-1;i++){connectedClients[i]=connectedClients[i+1];notifyEnabled[i]=notifyEnabled[i+1];}connectedClients[clientCount-1]=INVALID_CONN_HANDLE;notifyEnabled[clientCount-1]=false;if(clientCount>0)clientCount--;}if(bleRunning&&clientCount<MAX_BLE_CLIENTS)startBLEAdvertising();}else if(event->type==BLE_GAP_EVENT_ADV_COMPLETE){if(bleRunning)startBLEAdvertising();}else if(event->type==BLE_GAP_EVENT_SUBSCRIBE){int slot=bleClientSlot(event->subscribe.conn_handle);if(slot>=0&&event->subscribe.attr_handle==txHandle)notifyEnabled[slot]=event->subscribe.cur_notify;}return 0;}
...
static void startBLE(){if(bleRunning||!bleReady)return;bleRunning=true;startBLEAdvertising();}
static void initBLE(){if(nimble_port_init()!=ESP_OK){ESP_LOGE(TAG,"NimBLE initialization failed");return;}ble_svc_gap_init();ble_svc_gatt_init();if(bleInitServices()!=0){ESP_LOGE(TAG,"GATT service initialization failed");return;}ble_hs_cfg.sync_cb=bleSync;nimble_port_freertos_init(bleHostTask);}
static void stopBLE(){if(!bleRunning)return;(void)ble_gap_adv_stop();uint16_t handles[MAX_BLE_CLIENTS];int count=clientCount;if(count>MAX_BLE_CLIENTS)count=MAX_BLE_CLIENTS;for(int i=0;i<count;i++)handles[i]=connectedClients[i];bleRunning=false;for(int i=0;i<count;i++){if(handles[i]!=INVALID_CONN_HANDLE)(void)ble_gap_terminate(handles[i],BLE_ERR_REM_USER_CONN_TERM);}for(int i=0;i<MAX_BLE_CLIENTS;i++){connectedClients[i]=INVALID_CONN_HANDLE;notifyEnabled[i]=false;}clientCount=0;}
static void processBLEQueue(){if(!bleQueue||!bleRunning||clientCount==0)return;static uint64_t lastSend=0;if(nowMs()-lastSend<20)return;BleMessage message;if(xQueueReceive(bleQueue,&message,0)!=pdTRUE)return;for(int i=0;i<MAX_BLE_CLIENTS;i++){if(connectedClients[i]==INVALID_CONN_HANDLE||!notifyEnabled[i])continue;struct os_mbuf *om=ble_hs_mbuf_from_flat(message.data,strlen(message.data));if(!om)continue;int rc=ble_gatts_notify_custom(connectedClients[i],txHandle,om);if(rc!=0)os_mbuf_free_chain(om);}lastSend=nowMs();}
...
static void runStateMachine(){switch(state){case WIFI_START:{if(!wifiScanActive&&!wifiConnecting){ESP_LOGI(TAG,"Starting WiFi Scan");esp_wifi_disconnect();wifiBestIndex=performWifiScan();wifiScanActive=false;if(wifiBestIndex>=0){ESP_LOGI(TAG,"Connecting to %s (RSSI %d)",ssidList[wifiBestIndex],wifiBestRSSI);connectNetwork(wifiBestIndex);wifiConnecting=true;wifiConnectStart=nowMs();}else state=WIFI_STOPPING;}if(wifiConnecting){if(wifiConnected){wifiConnecting=false;state=WIFI_MODE;break;}if(nowMs()-wifiConnectStart>10000ULL){wifiConnecting=false;state=WIFI_STOPPING;}}break;}case WIFI_MODE:if(!wifiConnected)state=WIFI_STOPPING;else if(!mqttConnected&&nowMs()-lastMQTTRetry>5000ULL){connectMQTT();lastMQTTRetry=nowMs();}break;case WIFI_STOPPING:stopMQTT();esp_wifi_disconnect();wifiConnected=false;wifiConnecting=false;wifiScanActive=false;vTaskDelay(pdMS_TO_TICKS(200));state=BT_START;break;case BT_START:if(!bleReady)break;startBLE();state=BT_MODE;break;case BT_MODE:if(clientCount==0&&nowMs()-lastScanDone>15000ULL){ESP_LOGI(TAG,"BT_MODE: scanning for saved WiFi");wifiBestIndex=performWifiScan();lastScanDone=nowMs();if(wifiBestIndex>=0){stopBLE();btStopTimer=nowMs();state=BT_STOPPING;}}break;case BT_STOPPING:if(nowMs()-btStopTimer>500ULL)state=WIFI_START;break;}}
