...
static int bleGap(struct ble_gap_event *event,void*){if(event->type==BLE_GAP_EVENT_CONNECT){if(event->connect.status==0){if(clientCount>=MAX_BLE_CLIENTS)ble_gap_terminate(event->connect.conn_handle,BLE_ERR_CONN_LIMIT);else{connectedClients[clientCount]=event->connect.conn_handle;notifyEnabled[clientCount]=false;clientCount++;syncRequested=true;startBLEAdvertising();}}}else if(event->type==BLE_GAP_EVENT_DISCONNECT){int slot=bleClientSlot(event->disconnect.conn.conn_handle);if(slot>=0){for(int i=slot;i<clientCount-1;i++){connectedClients[i]=connectedClients[i+1];notifyEnabled[i]=notifyEnabled[i+1];}connectedClients[clientCount-1]=INVALID_CONN_HANDLE;notifyEnabled[clientCount-1]=false;if(clientCount>0)clientCount--;}if(bleRunning&&clientCount<MAX_BLE_CLIENTS)startBLEAdvertising();}else if(event->type==BLE_GAP_EVENT_SUBSCRIBE){int slot=bleClientSlot(event->subscribe.conn_handle);if(slot>=0&&event->subscribe.attr_handle==txHandle)notifyEnabled[slot]=event->subscribe.cur_notify;}return 0;}
