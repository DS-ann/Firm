...
void handleCommand(String cmd){
  ...
  if(cmd[0]>='0' && cmd[0]<='7' && (cmd[1]=='1' || cmd[1]=='0')){
    int id=cmd[0]-'0'; bool s=cmd[1]=='1';
    setRelay(id,s); sendRelayMsg();
  }
...
bool connectWiFi(){
  ...
  while(WiFi.status()!=WL_CONNECTED && millis()-start<10000){ digitalWrite(LED_WIFI,millis()%500<250?HIGH:LOW); delay(50); }
...
void runStateMachine(){
  switch(state){
    case WIFI_START: if(millis()-wifiRetryTimer<10000) return; wifiRetryTimer=millis();
      Serial.println("Starting WiFi"); if(connectWiFi()){ delay(100); connectMQTT(); state=WIFI_MODE; } break;
    case WIFI_MODE: if(WiFi.status()==WL_CONNECTED){
        if(!mqtt.connected() && millis()-lastMQTTRetry>5000){ connectMQTT(); lastMQTTRetry=millis(); } else mqtt.loop();
      } break;
    case WIFI_STOP: Serial.println("Stopping WiFi"); mqtt.disconnect(); WiFi.disconnect(true,true); espClient.stop(); delay(2000); state=BT_START; break;
    case BT_START: startBT(); state=BT_MODE; break;
    case BT_MODE: while(SerialBT.available()){ String cmd=SerialBT.readStringUntil('\n'); handleCommand(cmd); }
      // Scan WiFi in background every 10s
      if(millis()-lastWiFiScanBT>10000){ lastWiFiScanBT=millis(); int n=WiFi.scanNetworks();
        int bestRSSI=-999,bestSaved=-1;
        for(int i=0;i<n;i++){ String f=WiFi.SSID(i); int r=WiFi.RSSI(i);
          for(int j=0;j<NUM_WIFI;j++){ if(f==ssidList[j] && r>bestRSSI){ bestRSSI=r; bestSaved=j; } }
        }
        if(bestSaved!=-1){ Serial.println("WiFi found while BT active, switching..."); stopBT(); state=WIFI_START; }
      }
      break;
...
void setup(){
  ...
  prefs.begin("relay",false); for(int i=0;i<NUM_RELAYS;i++){ char k[10]; sprintf(k,"r%d",i); relayState[i]=prefs.getBool(k,false); digitalWrite(relayPins[i],relayState[i]?LOW:HIGH); }
  startupAnimation();
}
